...
SCT.title = "sct"
SCT.version = C_AddOns.GetAddOnMetadata(SCT.title, "Version")
...
local function GetSpellInfo(spellIdentifier)
	-- since tww 11.0.0
	if (C_Spell) and (C_Spell.GetSpellInfo) then
		...
		return spellInfo.name, _, spellInfo.iconID, spellInfo.castTime, spellInfo.minRange, spellInfo.maxRange, spellInfo.spellID, spellInfo.originalIconID;
	end
	
	-- before tww 11.0.0
	return _G.GetSpellInfo(spellIdentifier); -- [18.07.19] 8.0/BfA: 2nd param "rank/nameSubtext" now returns nil
end

local function GetSpellCooldown(spellIdentifier)
	-- since tww 11.0.0
...
function SCT:ShowMenu(makeBlizzOptionsOnly)
  local loaded, message = C_AddOns.LoadAddOn("sct_options")
  if (loaded) then
	...
	if (not makeBlizzOptionsOnly) then
	  OpenAddOnCategory("SCT", "SCT "..SCT.LOCALS.OPTION_MISC21.name)
	end
  else
...
function SCT:COMBAT_TEXT_UPDATE(event, larg1)
  ...
  if (larg1=="SPELL_ACTIVE") then
    ...
    if not self:CheckSkill(larg2.."!")  then
      local texture = GetSpellTexture(isWoWFlavor.ClassicEra and larg3 or larg2)
    ...
    if (GetTime() >= value[1] + 0.5) then
      ...
      if (value[2] == "spell") then
		getCooldownDetails = self:Memoize(function()
          local start, duration, enabled = GetSpellCooldown(isWoWFlavor.ClassicEra and value[4] or value[3])
		  return {
			name = (isWoWFlavor.ClassicEra and value[4] or GetSpellInfo(value[3])),
			texture = GetSpellTexture(isWoWFlavor.ClassicEra and value[4] or value[3]),
			start = start,
			duration = duration,
			enabled = enabled
		  }
...
function SCT:ParseReflect(timestamp, event, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, ...)
  ...
  local texture = GetSpellTexture(isWoWFlavor.ClassicEra and spellName or spellId)
  --reflected events
